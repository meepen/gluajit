...
LJLIB_CF(table_remove)		LJLIB_REC(.)
{
  ...
  if (!(1 <= pos && pos <= e))  /* Nothing to remove? */
    return 0;
  lua_rawgeti_hack(L, 1, pos);  /* Get previous value. */
  /* NOBARRIER: This just moves existing elements around. */
  for (; pos < e; pos++) {
...
LJLIB_CF(table_concat)
{
  ...
  if (i <= e) {
	...
	lj_err_callerv(L, LJ_ERR_TABCAT, lj_typename(o), i);
      ...
      if (i++ == e) break;
      if (seplen)
	luaL_addlstring_hack(&b, strdata(sep), seplen);
    }
  }
...
static int sort_comp(lua_State *L, int a, int b)
{
  if (!lua_isnil(L, 2)) {  /* function? */
    ...
    lua_pushvalue_hack(L, a-1);  /* -1 to compensate function */
    lua_pushvalue_hack(L, b-2);  /* -2 to compensate function and `a' */
    lua_call_hack(L, 2, 1);
